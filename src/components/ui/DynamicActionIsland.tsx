...
const DynamicIslandContainer = ({ children }: { children: ReactNode }) => {
    return (
        <div className="z-10 flex h-full w-full items-end justify-center bg-[#212329] p-2 rounded-full">
            {children}
        </div>
    )
}
...
const calculateDimensions = (
    ...
    if (isMassiveOnMobile) {
        return { width: "240px", height: MAX_HEIGHT_MOBILE_MASSIVE }
    }

    if (isUltraOnMobile) {
        return { width: "240px", height: MAX_HEIGHT_MOBILE_ULTRA }
    }
...
const DynamicIslandContent = ({
    ...
    return (
        <motion.div
            id={id}
            className="mx-auto h-0 w-0 items-center justify-center  bg-[#212329] text-center text-gray-200 transition duration-300 ease-in-out focus-within:bg-neutral-900 hover:shadow-md   dark:focus-within:bg-black"
            animate={{
